...
class Owlpress_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'owlpress-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'owlpress_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'owlpress_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$owlpress_customizer_notify_show_recommended_actions = get_option( 'owlpress_customizer_notify_show' );
		foreach ( $owlpress_customizer_notify_recommended_actions as $key => $owlpress_lite_customizer_notify_recommended_action ) {
			if ( $owlpress_customizer_notify_show_recommended_actions[ $owlpress_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $owlpress_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$owlpress_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $owlpress_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $owlpress_customizer_notify_recommended_action['plugin_slug'] );
				$owlpress_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $owlpress_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$owlpress_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$owlpress_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$owlpress_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$owlpress_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$owlpress_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$owlpress_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$owlpress_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$owlpress_lite_customizer_notify_recommended_action['button_label'] = $owlpress_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $owlpress_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$owlpress_lite_customizer_notify_show_recommended_plugins = get_option( 'owlpress_customizer_notify_show_recommended_plugins' );

		foreach ( $owlpress_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $owlpress_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $owlpress_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $owlpress_deactivate_button_label;
					break;
			}
		...
		$json['total_actions']       = count( $owlpress_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="owlpress-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="owlpress-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="owlpress-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="owlpress-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no owlpress-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="owlpress-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="owlpress-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no owlpress-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
